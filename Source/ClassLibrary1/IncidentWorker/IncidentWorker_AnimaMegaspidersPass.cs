...
{
    public class IncidentWorker_AnimaMegaspidersPass : IncidentWorker
    {
        protected override bool CanFireNowSub(IncidentParms parms)
        {
            Map target = (Map)parms.target;
            return !target.gameConditionManager.ConditionIsActive(GameConditionDefOf.ToxicFallout) && target.mapTemperature.SeasonAndOutdoorTemperatureAcceptableFor(ThingDefOf.AnimaMegaspider) && this.TryFindEntryCell(target, out IntVec3 _);
        }
        ...
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            ...
            if (!this.TryFindEntryCell(target, out cell))
                return false;
            PawnKindDef kindDef = PawnKindDefOf.AnimaMegaspider;
            ...
            this.SendStandardLetter("LetterLabelAnimaMegaspidersPass".Translate((NamedArgument)kindDef.label).CapitalizeFirst(), "LetterAnimaMegaspidersPass".Translate((NamedArgument)kindDef.label), LetterDefOf.PositiveEvent, parms, (LookTargets)(Thing)newThing);
            return true;
        }
